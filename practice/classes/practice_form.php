...
class practice_form extends moodleform {
    public function definition() {
        ...
        $mform->addRule('firstname', 'Firstname must not be blank', 'required', null, 'client');

        $mform->addElement('text','lastname','Last name');
        $mform->setType('lastname',PARAM_TEXT);
        $mform->addRule('lastname', 'Lastname must not be blank', 'required', null, 'client');

        $mform->addElement('text','email','Email');
        $mform->setType('email',PARAM_EMAIL);
        $mform->addRule('email', 'Not valid email', 'required', null, 'client');
